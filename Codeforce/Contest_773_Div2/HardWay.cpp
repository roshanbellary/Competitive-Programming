...
double solve(){
    ...
    if (pts[0].s==pts[1].s && pts[2].s<pts[0].s) return calcdist(0,1);
    else if (pts[0].s==pts[2].s && pts[1].s<pts[0].s) return calcdist(0,2);
    else if (pts[1].s==pts[2].s && pts[0].s<pts[1].s) return calcdist(1,2);
    return 0;
}
int main(){
	setIO();rv(T);
